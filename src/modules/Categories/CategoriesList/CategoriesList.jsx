...
export default function CategoriesList({ loginData }) {
  ...
  const editCategory = async (data) => {
    ...
    const ViewCategoryDetails =async(CategoryDetails)=>{
       try {
          let response = await axios.get(`https://upskilling-egypt.com:3006/api/v1/Category/${CategoryDetails.id}}`,
            {
  ...
  const deleteCategory = async (CategoryId) => {
    try {
      await axios.delete(`https://upskilling-egypt.com:3006/api/v1/Category/${CategoryId}`, {
        headers: {
          Authorization: localStorage.getItem('token')
        }
      });
  ...
  return (
    <>
      ...
      <div className="home-content mx-4 p-2 mt-4 ">
        ...
        <table className="table table-borderless table-striped table-responsive table-hover w-100">
          <thead className='p-3'>
            <tr>
              ...
              <th scope="col" className="text-center">CreationDate</th>
              <th scope="col" className="text-center">ModificationDate</th>
            ...
            {isLoading ? <Loading /> :
              categoriesList.length > 0 ? (
                categoriesList.map((item, index) => (
                  <tr key={index}>
                    <td>{item.name}</td>
                    <td className="text-center">{new Date(item.creationDate).toLocaleString()}</td>
                    <td className="text-center">{new Date(item.modificationDate).toLocaleString()}</td>
                    <td className="text-center">
                      <i className="fa fa-eye" onClick={()=>handleShowCategoryDetails(item)}></i>
                      <i className="fa fa-edit mx-2 text-warning" onClick={() => handleShowEdit(item)}></i>
                      <i className="fa fa-trash text-danger" onClick={() => handleShowDelete(item.id)}></i>
                    </td>
                  </tr>
                ))
              ) : (
        ...
        <Modal show={show} onHide={handleClose} dialogClassName="w-75" className='d-flex justify-content-center align-items-center'>
          ...
          <Modal.Body>
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <button className={`btn ${isEditMode? 'btn-warning':'btn-success'}  w-100 mt-2`}>{isEditMode ? 'Edit Category' : 'Add Category'}</button>
              <ToastContainer />
            </form>
          </Modal.Body>
        </Modal>
        ...
        <Modal show={showDelete} onHide={handleCloseDelete} className='d-flex justify-content-center align-items-center'>
          <Modal.Header closeButton></Modal.Header>
          <Modal.Body className=''>
            <DeleteConiformation deletitem={'Category'} />
          </Modal.Body>
          <Modal.Footer>
            <Button variant="btn btn-outline-danger" onClick={() => deleteCategory(CategoryId)}>
              Delete this item
            </Button>
          </Modal.Footer>
          <ToastContainer />
      ...
      >
        ...
        <Modal.Body className='p-5'>
                <div className="categoryDetails d-flex flex-column align-items-center justify-content-center">
                         ...
                         <p className='my-2'>  Creation Date: {new Date(CategoryDetails.creationDate).toLocaleString()}</p>
                         <p className=' text-center'>Modification Date: {new Date(CategoryDetails.modificationDate).toLocaleString()}</p>
                </div>
        </Modal.Body>
        <Modal.Footer>
